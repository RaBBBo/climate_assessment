...
for year in range(year_start, year_end + 1):
    ...
    params = {
        ...
        "geo": ["BE", "BG", "CZ", "DK", "DE", "EE", "IE", "EL", "ES", "FR", "HR", "IT", "CY", "LV", "LT", "LU", "HU",
                "MT", "NL", "AT", "PL", "PT", "RO", "SL", "SK", "FI", "SE", "IS", "NO", "CH", "UK", "BA", "MK", "AL",
                "RS", "TR", "XK"],
    }
    dataset = "env_wat_abs"
    ...
    df["Water process"] = df["Water process"].map(
        {'Water abstraction by agriculture, forestry, fishing': 'Water abstraction for agriculture',
         "Water abstraction by public water supply": "Water abstraction for public water supply"})
    EUROSTAT_ENV_WAT_ABS = df.merge(db_map_EUROSTAT, how="left",
                                    on=["Water process", "Water sources", "Unit of measure"])
    ...
    NR_data_all = NR_data_all.rename(
        columns={'country': "Country", 'Values': "Value", 'Heatmap_sector_level_3_cd': "Sector"})
    ...
    (common_list_abbr, common_list_full_name,
     [NR_data_all, ES_data_all, T_data_all, RM_data_all, OPEX_data_all, CAPEX_data_all, BA_data_all,
      WF_data_all]) = Remove_countries(
        [NR_data_all, ES_data_all, T_data_all, RM_data_all, OPEX_data_all, CAPEX_data_all, BA_data_all,
         WF_data_all])  # raw material
    ...
    def GVA_norm(VI,
        ...
        for group in conn_group:
            ...
            sum_GVA = select_GVA.groupby(by='Country', as_index=False)[
                'Value'].sum()  # it's not sum but just value by country
    ...
    NR_data_all_norm.to_csv((f'{output_path}/IntOutput_Normalisation_NR.csv'), sep=';', decimal=',',
                            float_format='%.2f')
    ES_data_all_norm.to_csv((f'{output_path}/IntOutput_Normalisation_ES.csv'), sep=';', decimal=',',
                            float_format='%.2f')
    RM_data_all_norm.to_csv((f'{output_path}/IntOutput_Normalisation_RM.csv'), sep=';', decimal=',',
                            float_format='%.2f')
    T_data_all_norm.to_csv((f'{output_path}/IntOutput_Normalisation_T.csv'), sep=';', decimal=',', float_format='%.2f')
    OPEX_data_all_norm.to_csv((f'{output_path}/IntOutput_Normalisation_OPEX.csv'), sep=';', decimal=',',
                              float_format='%.2f')
    CAPEX_data_all_norm.to_csv((f'{output_path}/IntOutput_Normalisation_CAPEX.csv'), sep=';', decimal=',',
                               float_format='%.2f')
    BA_data_all_norm.to_csv((f'{output_path}/IntOutput_Normalisation_BA.csv'), sep=';', decimal=',',
                            float_format='%.2f')
    WF_data_all_norm.to_csv((f'{output_path}/IntOutput_Normalisation_WF.csv'), sep=';', decimal=',',
                            float_format='%.2f')
